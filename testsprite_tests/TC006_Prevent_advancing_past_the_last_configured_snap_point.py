...
async def run_test():
    ...
    try:
        ...
        # Click Next snap more times than there are snap points — should stay open at the last snap
        for _ in range(3):
            await page.evaluate("document.getElementById('drawer-next-snap').click()")
            await page.wait_for_timeout(400)

        # Drawer must remain open, not closed or crashed
        await expect(drawer).to_have_attribute('data-state', 'open', timeout=5000)
