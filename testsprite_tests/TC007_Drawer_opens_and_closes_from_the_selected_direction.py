...
async def run_test():
    ...
    try:
        ...
        left_btn = page.locator('#direction-left-btn')

        # Set direction to left (button is outside the drawer — always accessible)
        await left_btn.click()
        await expect(drawer).to_have_attribute('data-vaul-drawer-direction', 'left', timeout=5000)

        await trigger.click()
        await expect(drawer).to_have_attribute('data-state', 'open', timeout=5000)
        await expect(drawer).to_have_attribute('data-vaul-drawer-direction', 'left', timeout=3000)
